...
if __name__ == "__main__":
    ...
    for global_step in range(args.total_timesteps):
        ...
        if "final_observation" in infos:
            # 2. 如果有，再遍历找出是哪个环境结束了
            for idx, (truncated, terminated) in enumerate(zip(truncations, terminations)):
                # 如果是因为超时(truncated)或结束(terminated)导致重置
    ...
    # 2. 创建一个新的“可见”环境 (render_mode="human")
    ...
    for episode in range(num_demo_episodes):
        # 3. 转化为 Tensor 并放入 GPU
        obs_tensor = torch.Tensor(obs).to(device).unsqueeze(0)#将一维数组变成二维的，才能用神经网络处理

        # 4. 让 Actor 决定动作
        # 注意：get_action 返回三个值 (随机动作, log概率, 确定性均值)
        # 在演示/测试时，我们通常使用 [2] 均值 (mean)，因为它是表现最稳定的
        with torch.no_grad():
            _, _, action = actor.get_action(obs_tensor)

        # 5. 转回 NumPy 给环境执行
        action = action.cpu().numpy()[0]#将输出变成一维的，才能被环境接收

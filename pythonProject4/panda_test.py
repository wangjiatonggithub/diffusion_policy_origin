...
def make_env(env_id, seed, idx, capture_video, run_name):
    def thunk():
        ...
        if capture_video and idx == 0:
            ...
            # env = gym.wrappers.FlattenObservation(env) # 作用是将环境返回的观测数据由字典变成一维数组，方便神经网络读取
        ...
        env = gym.wrappers.FlattenObservation(env)
        # gym.wrapper库的作用是修改智能体和环境之间的交互数据，以符合各个部分的格式
...
class Actor(nn.Module):
    def __init__(self, env): # 利用环境作为输入是为了获取状态和动作空间的维数
        super().__init__()
        self.fc1 = nn.Linear(np.array(env.single_observation_space.shape).prod(), 256) # prod表示乘积，用于计算神经元的个数，输入层维数自动匹配
        self.fc2 = nn.Linear(256, 256)
        ...
        self.register_buffer(
            "action_scale", # 真实的动作范围也可以根据环境自动适应
            torch.tensor(
    ...
    def forward(self, x):
        ...
        # 将方差经过tanh函数输出后拉伸平移到[LOG_STD_MIN,LOG_STD_MAX]范围，tanh函数的作用是防止神经网络输出的对数方差太大或太小，拉伸平移的作用是让对数方差映射到我们更关注的范围上
        return mean, log_std
    ...
    def get_action(self, x):
        ...
        x_t = normal.rsample()  # for reparameterization trick (mean + std * N(0,1))重参数化采样，使得动作a相对于均值和标准差可导
...
if __name__ == "__main__":
    ...
    # 设置tensorboard记录器，runs为tensorboard的名称
    writer = SummaryWriter(f"runs/{run_name}")
    # 将实验的所有超参数以markdown的形式写进tensorboard中，在tensorboard的TEXT栏可以看到
    writer.add_text(
        "hyperparameters",
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step < args.learning_starts:
            # 若还没到学习时间，就随机采样动作
            actions = np.array([envs.single_action_space.sample() for _ in range(envs.num_envs)]) # 这里action是末端执行器每一步的位置增量
        else:
        ...
        # # 输出关节力矩
        # # A. 获取底层机器人对象
        # # env.unwrapped 是最原始的环境，.robot 是机器人对象
        # sim = envs.envs[0].unwrapped.sim
        # robot = envs.envs[0].unwrapped.robot
        # # B. 获取物理客户端 ID (PyBullet Client ID)
        # physics_client = sim.physics_client
        # # C. 调用 PyBullet API 查询关节状态
        # # robot.body_name 是机器人模型ID, robot.joint_indices 是7个关节的索引
        # joint_states = physics_client.getJointStates(
        #     sim._bodies_idx[robot.body_name],  # <--- 这里传入 int
        #     robot.joint_indices,
        #     # physicsClientId=physics_client
        # )
        # # D. 提取力矩 (Applied Joint Motor Torque)
        # # getJointStates 返回一个元组列表，每个元组的第 3 项 (index 3) 是施加的力矩
    ...
    if user_input.lower() == 'y':
        ...
        for _ in range(step):
            ...
            with torch.no_grad():
                # 获取动作 (使用均值 mean 表现更好)
                _, _, mean = actor.get_action(obs_tensor) # 用到了get_action函数中的均值mean
